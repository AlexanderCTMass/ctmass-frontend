...
export const ProjectDescriptionStep = (props) => {
    ...
    return (
        <Stack
            ...
            <Alert severity="info">
                Optional, but if you describe the details, the specialists will better understand what is required of
                them and whether they are suitable for the job
            </Alert>
            <QuillEditor
                onChange={handleContentChange}
                placeholder="For ex.: Install 4 interior doors + office skirting"
                value={content}
                modules={modules}
                formats={formats}
                readOnly={submit}
            ...
            <ImageList
                ...
                rowHeight={101}
            >
                {existingAttach.map((url, idx) =>
                    <Preview key={url || idx} attach={{ preview: url }} onRemove={() => handleRemoveExistingPhotos(url)} />
                )}
                {attach.map((item, idx) => (
                    <Preview key={item.preview || idx} attach={item} onRemove={() => handleRemovePhotos(item.preview)}
            ...
            >
                <Button
                    ...
                    variant="contained"
                    disabled={submit} // Проверяем, что description заполнен
                >
                    Continue
                </Button>
                <Button
