...
export const ReviewRequestModal = ({
    ...
    const formik = useFormik({
        initialValues: {
            ...
            specialty: currentRequest.specialty || trade?.primarySpecialtyId || '',
            projectDescription: currentRequest.projectDescription || '',
            files: currentRequest.files || [],
            email: currentRequest.email || '',
            message: currentRequest.message || '',
            location: currentRequest.location || trade?.location?.address || ''
        },
        validationSchema,
        onSubmit: (values) => {
            INFO('Submitting review request for trade', values, trade);
            onSubmit(values);
            onClose();
            formik.resetForm();
            setActiveStep(0);
    ...
    const handleNext = () => {
        ...
        if (activeStep === 0) {
            ...
            isValid = !formik.errors.projectName && !formik.errors.date && !formik.errors.specialty && !formik.errors.location;
        }
